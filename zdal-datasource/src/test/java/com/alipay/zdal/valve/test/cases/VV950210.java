...
@Feature("table限流:单时间间隔内，执行次数大于限流次数，应被限")
public class VV950210 extends ValveBasicTest{
	...
	@Subject("表限流(BASIC1619)：参数{tbName,1,5}，第2次被限")
	...
	public void TC950211() {
		...
		try {
			...
			Assert.areEqual(2L,getRequestTime(),"是否第2次限流");
		} catch (SQLException e) {
	...
	@Subject("表限流：参数{tbName,0,5}，操作次数最小值，第1次被限")
	@Priority(PriorityLevel.NORMAL)
	@Tester("riqiu")
	@Test
	public void TC950212() {
		...
		try {
			...
			Assert.areEqual(OutstripValveException.class, e.getClass(),"验证异常类");
			Assert.areEqual(1L,getRequestTime(),"是否第1次限流");
		} catch (SQLException e) {
			Assert.isTrue(false, "SQLException");
		} catch (Exception e) {
			Assert.isTrue(false, "Exception");
	...
	@Subject("表限流：参数{tbName,2,1}，时间间隔最小值，第3次被限")
	@Priority(PriorityLevel.NORMAL)
	@Tester("riqiu")
	@Test
	public void TC950213() {
		...
		try {
			...
			Assert.areEqual(OutstripValveException.class, e.getClass(),"验证异常类");
			Assert.areEqual(3L, getRequestTime(),"是否第3次限流");
		} catch (SQLException e) {
			Assert.isTrue(false, "SQLException");
		} catch (Exception e) {
			Assert.isTrue(false, "Exception");
	...
	@Subject("表限流：参数{tbName,1,60}，时间间隔最大值，第2次被限")
	@Priority(PriorityLevel.NORMAL)
	@Tester("riqiu")
	@Test
	...
	@Subject("表限流(BASIC1620)：重复设置参数{tbName,1,5}，第2次被限")
	@Priority(PriorityLevel.NORMAL)
	@Tester("riqiu")
	@Test
	...
	@Subject("表限流：参数{tbName,2,60}，多操作次数+时间间隔最大值，第301被限")
	@Priority(PriorityLevel.NORMAL)
	@Tester("riqiu")
	@Test
	public void TC950216() {
		...
		try {
			...
			Assert.areEqual(OutstripValveException.class, e.getClass(),"验证异常类");
			Assert.areEqual(3L,getRequestTime(),"是否第301次限流");
		} catch (SQLException e) {
			Assert.isTrue(false, "SQLException");
		} catch (Exception e) {
			Assert.isTrue(false, "Exception");
