...
public class SR957040 {
	...
	@Subject("动态调整minConn值,调低。将最小连接数由原来的4调到2")
	@Priority(PriorityLevel.HIGHEST)
	@Test
	public void TC957041() {
		Step("1、min为4，连接创建完成，managed达到4");
		dynamicMinConn.testsleep(30000);
		PoolCondition pc = zz.getPoolCondition();
		Assert.areEqual(4, dynamicMinConn.checkPoint(pc), "min为4，连接创建完成，managed达到4");
		Step("调整min值为2");
		dynamicMinConn.resetMinConnection(2,zz);
		Step("2、min值为2，连接数销毁重建，managed变为2");
		dynamicMinConn.testsleep(60000);
		pc = zz.getPoolCondition();
		Assert.areEqual(2, dynamicMinConn.checkPoint(pc), "min值为2，连接数销毁重建，managed变为2");
	}
